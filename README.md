...
- **Email:** none for now (Resend is only needed if the newsletter is re-enabled)
- **Deployment:** Vercel
...
- There is no contact or newsletter form for now: the Contact page links to Instagram. The bot's newsletter tool switches on automatically once `RESEND_API_KEY` and `RESEND_AUDIENCE_ID` are set (it needs a signup form that saves to that audience, which was removed; see git history before commit "Hide contact and newsletter forms").
...
│   ├── api/                # API routes (telegram agent webhook, daily cron)
