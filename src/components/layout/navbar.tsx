...
export function Navbar({locale}: {locale: string}) {
  ...
  return (
    <header
      className={`fixed top-0 z-50 w-full transition-all duration-700 ${scrolled ? 'bg-background/85 backdrop-blur border-b border-border' : 'bg-transparent'}`}
    >
      ...
      {open && (
        <div className="md:hidden min-h-screen bg-background p-10">
          {items.map(([href, label]) => (
            <Link className="block py-5 text-2xl font-heading" key={href} href={`/${locale}${href}`} onClick={() => setOpen(false)}>
              {label}
            </Link>
          ))}
